...
MX           = nan*ones(NumTransMax, length(files), 42);
...
figure(24); clf;
for idx = [1:25,27:42]
   scatter( log(MX(:,:,26)), log(MX(:,:,idx)) , 'b','filled');
   xlabel('beta log'); ylabel([namevarsall{idx},' (ln)']);%ylabel(num2str(idx));
   box on; drawnow();
   pause();
end
